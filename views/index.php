...
<main class="grid-g-12">
    ...
    <!--CONTEUDO DO CIDADE E REGIÃO.-->
    <section class="grid-g-12 cidade-regiao conteudo-esquerdo">
        <h1 class="titulo">Noticias da cidade e região</h1>
        <?php
        $idcidade = funcoes::categoriaNome('cidade e regiao');
        $read->ExeRead('posts', "WHERE status = 1 AND id_cat = :idc ORDER BY data_criacao DESC LIMIT 6", "idc={$idcidade}");
        if (isset($read->getResultado()[0])):
            foreach ($read->getResultado()[0] as $resCid):
                ?>
                <article class="grid-g-4">
                    <a href="<?= BASE . 'artigo/' . $resCid['url'] ?>"><img src="<?= BASE . 'uploads/' . $resCid['capa'] ?>" alt="<?= $resCid['titulo'] ?>" title="<?= $resCid['titulo'] ?>" width="100%" height="200"></a>
                    <div class="bloco1-conteudo">
                        <h1><a href="<?= BASE . 'artigo/' . $resCid['url'] ?>"><?= funcoes::limtarTextos($resCid['titulo'], 50) ?></a></h1>
                        <p>
                            <?= funcoes::limtarTextos($resCid['conteudo'], 90) ?>  
                        </p>
                        <time><i class="fa fa-calendar"> <?= date('d-m-Y', strtotime($resCid['data_criacao'])) ?></i></time>
                        <p class="criado"><i class="fa fa-eye"> Visitas <?= $resCid['visitas'] ?></i></p>
                    </div>
                </article>
                <?php
            endforeach;
        else:
            echo '<div class="alert">Não contém posts relacionados ainda :( </div>';
        endif;
        ?>
    </section>
</main>
